...
private:
	...
	int sum_of_nodes(const BTNode<Item_Type>* local_root) const;
...
template<typename Item_Type>
int Binary_Tree<Item_Type>::sum_of_nodes(const BTNode<Item_Type>* local_root) const
{
	int sum = 0;
	int current_int;
	if (local_root != NULL)
	{
		current_int = stoi((local_root->to_string()));
		sum += current_int;
		if (local_root->left != NULL)
		{
			sum += 0;
			sum += sum_of_nodes(local_root->left);
		}
		if (local_root->right != NULL) {
			sum += 0;
			sum += sum_of_nodes(local_root->right);
		}
